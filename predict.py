...
if __name__ == "__main__":
    ...
    test_image_path = os.path.join('50_classes', 'test')### need modify
    ...
    model.cuda()
    file_list = [name for name in os.listdir(test_image_path) if
                   os.path.isdir(os.path.join(test_image_path, name))]
    file_list.sort()
    ...
    length_dict = pt_file['real_length_dict']  # get_length_dict('88log.txt')
    class2num = pt_file['class2num']
    num2class = dict((value, key) for key, value in class2num.items())
    update_n = 0
    with torch.no_grad():
        for ci, cf in enumerate(file_list):
            tmp = time.time()
            file_name_list.append(cf)
            img_path = os.path.join(test_image_path, cf)
            ...
            length_pred_list.append(new_preds)
            length_prob_list.append(new_probs)
            ori_pred_list.append(preds)
            ori_prob_list.append(probs)
            # print(f'inference time:{time.time() - tmp}')
    ...
    df = pd.DataFrame(
        {'img name': file_name_list,'ori_preds': ori_pred_list,
         'ori_probs': ori_prob_list, 'length_preds': length_pred_list,
         'length_probs': length_prob_list})
...
# print(
    # f'top1 length detection:{ld_top1_acc} %,top5 length detection:{ld_top5_acc} %,top7 length detection:{ld_top7_acc} %')
