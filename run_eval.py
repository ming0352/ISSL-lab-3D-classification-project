        ...
        for i in range(0, len(lines)):
            if 'Start taking photo for' in lines[i]:
                ...
                if class_name in length_dict.keys():
                    ...
                    length_dict[class_name]=max_length
    return length_dict
def length_detection(length_dict,cf,preds,probs):
    front_pred = []
    back_pred = []
    front_prob=[]
    back_prob=[]
    ...
    for i in range(0, len(preds[0])):
        if offset !=0:
            cmp_value=np.random.choice(np.arange(-offset, offset, 0.001)) + offset
        else:
            cmp_value=0.0
        if length_dict[num2class[int(preds[0][i])]] <= (taret_length + cmp_value):
...
if __name__ == "__main__":
    ...
    test_image_path = os.path.join('88_classes','mask_M11_real_test_image')
    ...
    total = 0
    n_samples = 0
    length_dict = get_length_dict('88log.txt')
